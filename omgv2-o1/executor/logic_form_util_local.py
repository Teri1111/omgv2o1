...
# from utils.sparql_execution import execute_query_with_odbc  # not needed for lisp_to_sparql
import re
import json
...
def lisp_to_sparql(lisp_program: str):
    ...
    for i, subp in enumerate(sub_programs):
        i = str(i)
        if subp[0] == 'JOIN':
            if isinstance(subp[1], list):  # R relation = forward = entity is SUBJECT
                if subp[2][:2] in ["m.", "g."]:  # entity
                    clauses.append("?x" + i + " ns:" + subp[1][1] + " ns:" + subp[2] + " .")
                    entities.add(subp[2])
                elif subp[2][0] == '#':  # variable
                    clauses.append("?x" + i + " ns:" + subp[1][1] + " ?x" + subp[2][1:] + " .")
            ...
            else:  # no-R = backward = entity is OBJECT
                if subp[2][:2] in ["m.", "g."]:  # entity
                    clauses.append("ns:" + subp[2] + " ns:" + subp[1] + " ?x" + i + " .")
                    entities.add(subp[2])
                elif subp[2][0] == '#':  # variable
                    clauses.append("?x" + subp[2][1:] + " ns:" + subp[1] + " ?x" + i + " .")
