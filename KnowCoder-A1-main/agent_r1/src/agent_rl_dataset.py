...
class ToolRLDataset(RLHFDataset):
    ...
    def __init__(
        ...
        # 支持从指定的文件索引开始加载数据
        resume_from_file_index = config.get("resume_from_file_index", 0)
        try:
            # 兼容 Hydra 的 ListConfig 类型
        ...
        # 仅对训练集生效，验证集不截断
        if (not is_validation) and resume_from_file_index > 0 and isinstance(data_files, (list, tuple)):
            if resume_from_file_index >= len(data_files):
                raise ValueError(f"resume_from_file_index {resume_from_file_index} >= number of data files {len(data_files)}")
            original_data_files = list(data_files)
            data_files = list(data_files)[resume_from_file_index:]
            print(f"[RESUME] 从第 {resume_from_file_index} 个数据文件开始加载: {data_files}")
            print(f"[RESUME] 跳过的文件: {original_data_files[:resume_from_file_index]}")
