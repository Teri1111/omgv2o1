...
class TaskRunner:
    def run(self, config):
        ...
        train_dataset = create_rl_dataset(config.data.train_files, config.data, tokenizer, processor, env=env, is_validation=False)
        if val_env is not None:
            val_dataset = create_rl_dataset(config.data.val_files, config.data, tokenizer, processor, env=val_env, is_validation=True)
        else:
            val_dataset = create_rl_dataset(config.data.val_files, config.data, tokenizer, processor, env=env, is_validation=True)
...
def create_rl_dataset(data_paths, data_config, tokenizer, processor, env=None, is_validation=False):
    """Create a dataset.
