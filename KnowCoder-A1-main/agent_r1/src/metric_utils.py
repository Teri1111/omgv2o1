...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    # 添加关系幻觉惩罚监测指标
    if "relation_hallucination_penalty" in batch.non_tensor_batch:
        rel_penalty = batch.non_tensor_batch["relation_hallucination_penalty"]
        # 只计算非零惩罚的统计信息
        nonzero_rel_penalty = rel_penalty[rel_penalty != 0]
        if len(nonzero_rel_penalty) > 0:
            metrics.update(
                {
                    "critic/relation_hallucination_penalty/mean": np.mean(rel_penalty),
                    "critic/relation_hallucination_penalty/nonzero_mean": np.mean(nonzero_rel_penalty),
                    "critic/relation_hallucination_penalty/min": np.min(rel_penalty),
                    "critic/relation_hallucination_penalty/max": np.max(rel_penalty),
                    "critic/relation_hallucination_penalty/count": len(nonzero_rel_penalty),
                    "critic/relation_hallucination_penalty/ratio": len(nonzero_rel_penalty) / len(rel_penalty),
                }
            )
        else:
            metrics.update(
                {
                    "critic/relation_hallucination_penalty/mean": 0.0,
                    "critic/relation_hallucination_penalty/nonzero_mean": 0.0,
                    "critic/relation_hallucination_penalty/min": 0.0,
                    "critic/relation_hallucination_penalty/max": 0.0,
                    "critic/relation_hallucination_penalty/count": 0,
                    "critic/relation_hallucination_penalty/ratio": 0.0,
                }
            )

    # 添加超时惩罚监测指标
    if "timeout_penalty" in batch.non_tensor_batch:
        timeout_penalty = batch.non_tensor_batch["timeout_penalty"]
        # 只计算非零惩罚的统计信息
        nonzero_timeout_penalty = timeout_penalty[timeout_penalty != 0]
        if len(nonzero_timeout_penalty) > 0:
            metrics.update(
                {
                    "critic/timeout_penalty/mean": np.mean(timeout_penalty),
                    "critic/timeout_penalty/nonzero_mean": np.mean(nonzero_timeout_penalty),
                    "critic/timeout_penalty/min": np.min(timeout_penalty),
                    "critic/timeout_penalty/max": np.max(timeout_penalty),
                    "critic/timeout_penalty/count": len(nonzero_timeout_penalty),
                    "critic/timeout_penalty/ratio": len(nonzero_timeout_penalty) / len(timeout_penalty),
                }
            )
        else:
            metrics.update(
                {
                    "critic/timeout_penalty/mean": 0.0,
                    "critic/timeout_penalty/nonzero_mean": 0.0,
                    "critic/timeout_penalty/min": 0.0,
                    "critic/timeout_penalty/max": 0.0,
                    "critic/timeout_penalty/count": 0,
                    "critic/timeout_penalty/ratio": 0.0,
                }
            )
