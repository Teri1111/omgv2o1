...
class RayAgentTrainer(object):
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler):
        ...
        if train_dataset is None:
            train_dataset = create_rl_dataset(self.config.data.train_files, self.config.data, self.tokenizer, self.processor, self.env, is_validation=False)
        if val_dataset is None:
            val_dataset = create_rl_dataset(self.config.data.val_files, self.config.data, self.tokenizer, self.processor, self.val_env, is_validation=True)
    ...
    def _load_checkpoint(self):
        ...
        # 如果指定了 resume_from_file_index，则不加载数据加载器状态，从新的文件开始
        resume_from_file_index = self.config.data.get("resume_from_file_index", 0)
        if resume_from_file_index > 0:
            print(f"[RESUME] 指定了 resume_from_file_index={resume_from_file_index}，跳过数据加载器状态恢复，从新文件开始")
        elif os.path.exists(dataloader_local_path):
            dataloader_state_dict = torch.load(dataloader_local_path, weights_only=False)
            self.train_dataloader.load_state_dict(dataloader_state_dict)
            print(f"[RESUME] 已恢复数据加载器状态从 {dataloader_local_path}")
        else:
    ...
    def _compute_zero_advantage_metrics(self, batch: DataProto) -> dict:
        """
        计算整个rollout组优势都为0的样本统计指标。
        
        在GRPO中，当一个rollout组内所有样本的奖励都相同时，优势会被设置为0。
        这个指标帮助监控有多少样本组没有足够的奖励差异来进行有效的策略梯度更新。
        
        Args:
            batch: 包含优势信息的数据批次
            
        Returns:
            metrics: 包含零优势组统计信息的字典
        """
        if "advantages" not in batch.batch:
            return {}
            
        advantages = batch.batch["advantages"]  # shape: (batch_size, response_length)
        action_mask = batch.batch["action_mask"]  # shape: (batch_size, response_length)
        uid_index = batch.non_tensor_batch.get("uid", None)
        
        if uid_index is None:
            return {"rollout_groups/zero_advantage_groups_count": 0,
                    "rollout_groups/zero_advantage_groups_ratio": 0.0}
        
        metrics = {}
        
        # 计算每个样本的有效优势（只考虑action tokens）
        sample_advantages = (advantages * action_mask).sum(dim=-1)  # shape: (batch_size,)
        ...
        for i in range(batch_size):
            group_id = uid_index[i]
            group_advantages[group_id].append(sample_advantages[i].item())
        
        # 统计完全零优势的组
        zero_advantage_groups = 0
        total_groups = len(group_advantages)
        
        for group_id, group_advs in group_advantages.items():
            # 检查这个组内所有样本的优势是否都接近0
            max_abs_advantage = max(abs(adv) for adv in group_advs)
            if max_abs_advantage < 1e-6:  # 基本为0的阈值
                zero_advantage_groups += 1
        
        # 计算比例
        zero_advantage_ratio = zero_advantage_groups / total_groups if total_groups > 0 else 0.0
        
        metrics.update({
            "rollout_groups/zero_advantage_groups_count": zero_advantage_groups,
            "rollout_groups/total_groups_count": total_groups,
            "rollout_groups/zero_advantage_groups_ratio": zero_advantage_ratio,
            "rollout_groups/effective_groups_count": total_groups - zero_advantage_groups,
            "rollout_groups/effective_groups_ratio": 1.0 - zero_advantage_ratio,
        })
        
        # 额外统计：计算优势分布信息
        all_advantages = [adv for group_advs in group_advantages.values() for adv in group_advs]
        if all_advantages:
            import numpy as np
            ...
            metrics.update({
                ...
                "advantages/min": float(np.min(advantages_array)),
                "advantages/max": float(np.max(advantages_array)),
                "advantages/abs_mean": float(np.mean(np.abs(advantages_array))),
            })
    ...
    def fit(self):
        ...
        # 如果从 checkpoint 恢复后，当前 global_steps 已经大于或等于预设的 total_training_steps，
        # 且用户没有在配置中显式指定 trainer.total_training_steps，则自动扩展总步数，避免训练立即结束。
        if (
            self.config.trainer.get("total_training_steps", None) is None
            and self.global_steps >= self.total_training_steps
        ):
            steps_per_epoch = len(self.train_dataloader)
            planned_steps = steps_per_epoch * self.config.trainer.total_epochs
            new_total_training_steps = self.global_steps + planned_steps
            print(
                f"[RESUME] Detected global_steps={self.global_steps} >= total_training_steps={self.total_training_steps}. "
                f"Auto-extending total_training_steps to {new_total_training_steps}."
            )
            self.total_training_steps = new_total_training_steps
            try:
                from omegaconf import OmegaConf, open_dict
                OmegaConf.set_struct(self.config, True)
                with open_dict(self.config):
                    if OmegaConf.select(self.config, "actor_rollout_ref.actor.optim"):
                        self.config.actor_rollout_ref.actor.optim.total_training_steps = new_total_training_steps
                    if OmegaConf.select(self.config, "critic.optim"):
                        self.config.critic.optim.total_training_steps = new_total_training_steps
            except Exception as e:
                print(f"Warning: Failed to inject extended total_training_steps into optim configs: {e}")
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        # 计算零优势组监控指标
                        zero_advantage_metrics = self._compute_zero_advantage_metrics(batch)
