...
class ToolGenerationManager:
    ...
    def run_llm_loop(self, gen_batch, env: Union[BaseToolEnv, BaseImageToolEnv]) -> Tuple[Dict, Dict]:
        ...
        for i, action_mask in enumerate(rollings.non_tensor_batch['action_mask']):
            ...
            # action_mask_slice = action_mask[:mask_len]
            # action_mask_slice_typed = action_mask_slice.astype(bool)
            # final_output['action_mask'][i, :mask_len] = torch.tensor(action_mask_slice_typed) * response_mask[i, :mask_len]
