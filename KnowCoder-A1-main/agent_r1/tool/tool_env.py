...
def step_batch(envs: List['ToolEnv'], action_texts: List[str]):
    ...
    t01 = time.time()
    print("[DEBUG] in tool env step batch, before tool execute loop consume:", t01-t00)
    ...
    for tool_name, args_list in tool_groups.items():
        ...
        t1 = time.time()
        print(f"[DEBUG] in tool env step_batch {tool_name}, index {i} step consume:", t1 - t0, args_list)
        ...
        for idx, env, result, args in zip(indices, envs_list, batch_results, args_list):
            ...
            t1 = time.time()
            print(f"[DEBUG] in tool env step_batch post-process , index {idx} step consume:", t1 - t0)
    t11 = time.time()
    print('[DEBUG] in tool env step_batch, tool total consume: ', t11-t00)
    return results
