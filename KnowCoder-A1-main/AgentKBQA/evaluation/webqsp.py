...
def cal_metrics(golden_answers: List[List[str]], predictions: List[List[str]]):
    ...
    digit = 4
    formatted_results = [
            round(ave_pre, digit),          # ave_pre 保留两位小数
            round(ave_rec, digit),          # ave_rec 保留两位小数
            round(average_f1, digit),       # average_f1 保留两位小数
            round(f1_average, digit),       # f1_average 保留两位小数
            round(acc, digit),              # acc 保留两位小数
            round(hits[0], digit),          # hits[0] 保留两位小数
            round(hits[1], digit),          # hits[1] 保留两位小数
            round(hits[2], digit),          # hits[2] 保留两位小数
            round(average_random_hit, digit)  # average_random_hit 保留两位小数
        ]
