...
from utils.executor.sparql_executor import execute_query
import re
import json
...
reverse_properties = {}
with open('./dataset/Freebase/reverse_properties', 'r') as f:
    for line in f:
...
def load_nameid_dict(file_dir, lower):
    ...
    file_cache = file_dir + '.pkl'
    if os.path.exists(file_cache):
       name2id_dict, id2name_dict = pickle.load(open(file_cache, 'rb')) 
    ...
    pickle.dump((name2id_dict, id2name_dict), open(file_cache, 'wb'))           
    return name2id_dict, id2name_dict

def logical_form_to_graph(expression: List, friendly_name=True) -> nx.MultiGraph:
    global id2name_dict
    if friendly_name and id2name_dict is None:
        name2id_dict, id2name_dict = load_nameid_dict('./dataset/Freebase/id2name_parts', lower=False)
        print("Loading name2id and id2name finished...")
