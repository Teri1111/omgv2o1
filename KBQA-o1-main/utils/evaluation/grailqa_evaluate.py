from utils.components.utils import dump_json, load_json
import os
...
def grailqa_convert_valid_results(pred_file):
    pred_data = load_json(pred_file)
    # Write answer acc result to prediction file
    converted_data = {pred['ID']:{"logical_form":pred['pred_sexpr'],"answer":pred["pred_answer"]} for pred in pred_data}
    dirname = os.path.dirname(pred_file)
    filename = os.path.basename(pred_file)
    dump_json(converted_data, os.path.join(dirname, f'{filename}_convert.json'), indent=4)
    
def grailqa_evaluate_valid_results(pred_file, gold_file):
    
    data = load_json(gold_file)
    predict = load_json(pred_file)
    predict = {str(pred['ID']):{"logical_form":pred['pred_sexpr'],"answer":pred["pred_answer"]} for pred in predict}
    
    fb_roles = 'dataset/Freebase/fb_roles'
    fb_types = 'dataset/Freebase/fb_types'
    reverse_properties = 'dataset/Freebase/reverse_properties'
    ...
    for item in data:
        ...
        if str(item['ID']) in predict:
            em = matcher.same_logical_form(predict[str(item['ID'])]['logical_form'], item['sexpr'])
            em_sum += em
            ...
            if em:
                ...
                predict_answer = set(predict[str(item['ID'])]['answer'])
    ...
    stats = {}
    stats['em'] = em_sum / len(data) if len(data) > 0 else 0.0
    stats['f1'] = f1_sum / len(data) if len(data) > 0 else 0.0
    stats['em_iid'] = level_em_sum['i.i.d.'] / level_count['i.i.d.'] if 'i.i.d.' in level_count else 0.0
    stats['f1_iid'] = level_f1_sum['i.i.d.'] / level_count['i.i.d.'] if 'i.i.d.' in level_count else 0.0
    stats['em_comp'] = level_em_sum['compositional'] / level_count['compositional'] if 'compositional' in level_count else 0.0
    stats['f1_comp'] = level_f1_sum['compositional'] / level_count['compositional'] if 'compositional' in level_count else 0.0
    stats['em_zero'] = level_em_sum['zero-shot'] / level_count['zero-shot'] if 'zero-shot' in level_count else 0.0
    stats['f1_zero'] = level_f1_sum['zero-shot'] / level_count['zero-shot'] if 'zero-shot' in level_count else 0.0

    print(stats)
    dirname = os.path.dirname(pred_file)
    filename = os.path.basename(pred_file)
    dump_json(stats, os.path.join(dirname, f'{filename}_final_eval_results.json'), indent=4)
